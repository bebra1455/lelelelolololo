--[[
    MegolaHub | MMV Script
    GUI: RightShift
    Дизайн в стиле Celestial
]]
...
local function HasValue(v)
    ...
    local s = v:lower()
    if s == "" or s == "none" or s == "nil" or s == "null" or s == "false" then return false end
    return true
end
...
local function GetRoleFromInfo(info)
    ...
    local hasKnife = HasValue(info.Knife)
    local hasGun = HasValue(info.Gun)
    ...
    if role:find("hero") or role:find("innocent") or role:find("civilian") then
    ...
    if hasGun and not hasKnife then return "Sheriff" end
    if hasKnife and hasGun then return "Murderer" end
...
local Skyboxes = {
    ...
    Evening = {
        ...
        Brightness = 1.5,
        ClockTime = 18,
        Ambient = Color3.fromRGB(100, 80, 80),
        OutdoorAmbient = Color3.fromRGB(90, 70, 70),
        FogEnd = 1000,
        FogColor = Color3.fromRGB(150, 100, 80),
    },
    Sunset = {
        SkyboxBk = "rbxassetid://105092364",
        SkyboxDn = "rbxassetid://105092385",
        SkyboxFt = "rbxassetid://105092306",
        SkyboxLf = "rbxassetid://105092413",
        SkyboxRt = "rbxassetid://105092351",
        SkyboxUp = "rbxassetid://105092442",
        Brightness = 2,
        ClockTime = 17,
    ...
    Anime = {
        ...
        SkyboxDn = "rbxassetid://6444884951",
        SkyboxFt = "rbxassetid://6444884415",
        SkyboxLf = "rbxassetid://6444883914",
        SkyboxRt = "rbxassetid://6444883684",
        SkyboxUp = "rbxassetid://6444885256",
        Brightness = 3,
        ClockTime = 12,
        Ambient = Color3.fromRGB(200, 200, 255),
        OutdoorAmbient = Color3.fromRGB(180, 180, 255),
        FogEnd = 5000,
...
ShaderDOF.Name = "MegolaHub_Shader"
...
local function ToggleShaders(enabled)
    ...
    TweenService:Create(ShaderDOF, TweenInfo.new(0.4), {FarIntensity = enabled and 1 or 0}):Play()
end
...
-- ============================================================
-- AUTO GUN LOOTER (гибридное сканирование)
-- ============================================================
...
local function UpdateGunDropCache()
    ...
    -- 3. Рекурсивно на 2 уровня вложенности (папки/модели напрямую в workspace)
...
local function TeleportGunToMe(gunDrop, myRoot)
    ...
    elseif gunDrop:IsA("BasePart") then
        if gunDrop.Anchored then
            pcall(function() gunDrop.Anchored = false end)
        end
...
local function CreateCard(category, name, defaultState, callback)
...
local function CreateSliderCard(category, name, min, max, default, callback)
    local card = Instance.new("Frame")
    card.Name = name .. "_Card"
    card.BackgroundColor3 = Colors.CardBackground
    card.BackgroundTransparency = Colors.CardTransparency
...
local function CreateBindCard(category, name, callback)
    local card = Instance.new("Frame")
    card.Name = name .. "_Card"
    card.BackgroundColor3 = Colors.CardBackground
    card.BackgroundTransparency = Colors.CardTransparency
...
print("MegolaHub загружен! Нажмите RightShift для открытия GUI.")
